...
### 5. Messagerie — parcours non confirmé de bout en bout

Le point d'entrée (bouton « Envoyer un message » depuis l'annuaire) et l'état vide de la messagerie ont été confirmés à plusieurs reprises par capture d'écran. L'envoi et la réception effectifs d'un message n'ont pas pu être menés à leur terme dans cette session d'automatisation, pour des raisons qui semblent liées à l'outillage de test plutôt qu'à la plateforme elle-même (des vérifications directes de l'API ont montré des données toujours cohérentes). **Statut : TESTÉ MAIS PARTIEL — une nouvelle vérification manuelle est recommandée avant de considérer cette fonctionnalité comme pleinement validée.**
