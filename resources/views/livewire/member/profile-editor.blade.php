...
<x-member.dark-page title="Mon profil" subtitle="Gérez vos informations personnelles et votre visibilité dans le réseau." icon="user">
    <div class="max-w-[680px]">
        ...
        <div class="mb-8 flex items-center gap-[18px] rounded-2xl border border-[var(--ms-bc2)] bg-[var(--ms-surf)] px-6 py-5">
            @if ($photo)
                <img src="{{ $photo->temporaryUrl() }}" class="size-16 shrink-0 rounded-full object-cover" alt="" />
            @elseif ($user->photo)
                <img src="{{ \Illuminate\Support\Facades\Storage::url($user->photo) }}" class="size-16 shrink-0 rounded-full object-cover" alt="" />
            @else
            ...
            <label class="shrink-0 cursor-pointer rounded-lg border border-[var(--ms-bc2)] bg-[var(--ms-surf2)] px-3.5 py-2 text-xs font-semibold text-[var(--ms-text)]">
                Changer la photo
                <input type="file" wire:model="photo" accept="image/*" class="hidden" />
            </label>
        </div>
        @error('photo') <p class="mb-4 text-xs font-medium text-[#E84A43]">{{ $message }}</p> @enderror
