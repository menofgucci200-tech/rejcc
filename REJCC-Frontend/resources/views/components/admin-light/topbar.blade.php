...
<header class="sticky top-0 z-50 flex h-16 items-center gap-4 border-b border-brand/10 border-t-[3px] border-t-accent bg-white/85 px-7 backdrop-blur-lg">
    ...
    <span class="relative flex size-10 shrink-0 items-center justify-center rounded-[10px] border border-brand/10 bg-white">
        <x-ui.icon name="bell" class="size-[18px] text-brand" />
        @if ($attention > 0)
            <span class="absolute -right-1 -top-1 flex h-[17px] min-w-[17px] items-center justify-center rounded-full border-2 border-white bg-accent px-1 text-[10px] font-bold text-white">{{ $attention }}</span>
        @endif
    </span>

    <div class="flex shrink-0 items-center gap-2.5 rounded-xl border border-brand/10 bg-white py-[5px] pl-3 pr-1.5">
        ...
        <div class="flex size-9 shrink-0 items-center justify-center rounded-[10px] text-xs font-bold text-white" style="background: linear-gradient(135deg, #AC0100, #D95B5A)">
