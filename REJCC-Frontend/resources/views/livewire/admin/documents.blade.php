<div>
    ...
    <div class="mx-auto max-w-[1280px] px-8 py-8">
        ...
        @if ($showForm)
            <div class="mb-6 grid grid-cols-1 gap-3.5 rounded-[18px] border border-brand/10 bg-white p-[22px] shadow-[0_2px_8px_rgba(3,29,89,.05)] sm:grid-cols-2">
                ...
                <div class="sm:col-span-2">
                    <label class="mb-1 block text-xs font-semibold text-[#5B677A]">URL du fichier</label>
                ...
                <div>
                    <label class="mb-1 block text-xs font-semibold text-[#5B677A]">Taille (ex : 2.4 Mo)</label>
        ...
        <div class="flex flex-col gap-8">
                        ...
                        @foreach ($docs->where('category', $cat) as $d)
                            <div class="flex items-center gap-3 rounded-[16px] border border-brand/10 bg-white p-4 shadow-[0_2px_8px_rgba(3,29,89,.05)]">
                                ...
                                <div class="min-w-0 flex-1">
                                    <p class="truncate text-[13px] font-bold text-brand">{{ $d->title }}</p>
                                    @if ($d->description)
                                        <p class="truncate text-xs text-[#5B677A]">{{ $d->description }}</p>
                                    @endif
                                </div>
            ...
            @empty
                <p class="py-12 text-center text-sm text-[#5B677A]">Aucun document.</p>
            @endforelse
        </div>
    </div>
</div>
