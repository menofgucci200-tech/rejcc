            ...
            @forelse ($candidatures as $c)
                ...
                <div class="flex flex-wrap items-center gap-4 border-t border-[#EDF0F5] py-4 first:border-t-0">
                    ...
                    <div class="flex shrink-0 items-center gap-2">
                        <button wire:click="toggleDetail({{ $c->id }})" class="inline-flex items-center gap-1.5 rounded-lg border border-azure/25 px-3.5 py-1.5 text-xs font-semibold text-azure hover:bg-azure/10">
                            <x-ui.icon name="eye" class="size-3.5" /> {{ $expanded === $c->id ? 'Masquer' : 'Voir' }}
                        </button>
                        @if (($c->statut ?? 'en_attente') === 'en_attente')
                            <button wire:click="accept({{ $c->id }})" wire:confirm="Approuver la demande de {{ $c->prenom }} {{ $c->nom }} ? Son compte membre sera créé et un email de bienvenue lui sera envoyé." class="inline-flex items-center gap-1.5 rounded-lg bg-[#22A85A] px-3.5 py-1.5 text-xs font-bold text-white hover:bg-[#1C8F4C]">
                                <x-ui.icon name="check" class="size-3.5" /> Approuver
                            </button>
                            <button wire:click="openReject({{ $c->id }})" class="inline-flex items-center gap-1.5 rounded-lg border border-[#F0C9C9] px-3.5 py-1.5 text-xs font-bold text-accent hover:bg-[#F9E9E9]">
                ...
                @if ($rejectingId === $c->id)
                    <div class="mb-4 rounded-[12px] border border-[#F0C9C9] bg-[#FDF7F7] p-4">
                        ...
                        <div class="mt-3 flex gap-2">
                            <button wire:click="confirmReject" wire:loading.attr="disabled" class="rounded-lg bg-accent px-4 py-1.5 text-xs font-bold text-white hover:bg-accent-600 disabled:opacity-60">Confirmer le rejet</button>
                            <button wire:click="cancelReject" class="rounded-lg border border-[#C9D3E6] px-4 py-1.5 text-xs font-bold text-brand hover:bg-cloud">Annuler</button>
                        </div>
                    </div>
                @endif
                @if ($expanded === $c->id)
