                    ...
                    <div class="flex flex-wrap items-center gap-4 border-t border-[#EDF0F5] py-4 first:border-t-0">
                        ...
                        @if (($c->statut ?? 'en_attente') === 'en_attente')
                            <div class="flex shrink-0 gap-2">
                                <button wire:click="accept({{ $c->id }})" wire:confirm="Accepter la candidature de {{ $c->prenom }} {{ $c->nom }} ? Son compte membre sera créé." class="rounded-lg bg-[#22A85A] px-3.5 py-1.5 text-xs font-bold text-white hover:bg-[#1C8F4C]">Accepter</button>
                                <button wire:click="openReject({{ $c->id }})" class="rounded-lg border border-[#F0C9C9] px-3.5 py-1.5 text-xs font-bold text-accent hover:bg-[#F9E9E9]">Refuser</button>
                            </div>
                        @endif
                    </div>
                    @if ($rejectingId === $c->id)
                        <div class="mb-4 rounded-[12px] border border-[#F0C9C9] bg-[#FDF7F7] p-4">
                            <p class="mb-2 text-[13px] font-bold text-accent">Refuser la candidature de {{ $c->prenom }} {{ $c->nom }}</p>
                            <label class="mb-1 block text-xs font-semibold text-[#5B677A]">Motif du refus (communiqué au candidat par email — optionnel)</label>
                            ...
                            <div class="mt-3 flex gap-2">
                                <button wire:click="confirmReject" wire:loading.attr="disabled" class="rounded-lg bg-accent px-4 py-1.5 text-xs font-bold text-white hover:bg-accent-600 disabled:opacity-60">Confirmer le refus</button>
