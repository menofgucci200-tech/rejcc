                ...
                <div class="row-hover -mx-5 flex flex-wrap items-center gap-4 border-t border-[#EDF0F5] px-5 py-4 first:border-t-0">
                    ...
                    @if ($d->statut === 'nouveau')
                        <div class="flex shrink-0 gap-2">
                            <button wire:click="accepter({{ $d->id }})" class="btn-tap rounded-lg bg-[#22A85A] px-3.5 py-1.5 text-xs font-bold text-white hover:bg-[#1C8F4C]">Accepter</button>
                            <button wire:click="refuser({{ $d->id }})" class="btn-tap rounded-lg border border-[#F0C9C9] px-3.5 py-1.5 text-xs font-bold text-accent hover:bg-[#F9E9E9]">Refuser</button>
                        </div>
                    @endif
                </div>
            @empty
