    ...
    <div class="overflow-x-auto rounded-2xl border border-brand/10 bg-white">
        <table class="w-full text-sm">
            <thead class="border-b border-brand/10 bg-cloud/60">
                <tr>
                    @foreach (['Nom', 'Contact', 'Statut actuel', 'Défi principal', 'Date', 'Statut', 'Actions'] as $h)
            ...
            <tbody class="divide-y divide-brand/5">
                @foreach ($applications as $a)
                    <tr class="hover:bg-cloud/50">
                        ...
                        <td class="px-4 py-3">
                            @php
                                $badge = match ($a->statut ?? 'en_attente') {
                                    'accepte' => ['bg-green-100 text-green-700', 'Acceptée'],
                                    'refuse' => ['bg-accent/10 text-accent', 'Refusée'],
                                    default => ['bg-brand/10 text-brand', 'En attente'],
                                };
                            @endphp
                            <span class="inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-semibold {{ $badge[0] }}">{{ $badge[1] }}</span>
                        </td>
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-1.5">
                                @if (($a->statut ?? 'en_attente') === 'en_attente')
                                    <button
                                        wire:click="accept({{ $a->id }})"
                                        title="Accepter"
                                    ...
                                    <button
                                        ...
                                        wire:confirm="Refuser cette candidature ?"
                                        title="Refuser"
                                        class="rounded-lg p-1.5 text-accent transition hover:bg-accent/10"
                                    >
                ...
                <dl class="grid gap-4 sm:grid-cols-2">
                    @php
                        $rows = [
                            ...
                            'Ville' => $viewingApplication->ville,
